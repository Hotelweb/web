...
export const getHotelBySlug = (slug: string) =>
  fetchApi<Hotel>(`/hotels/slug/${slug}`);
export const getHotelByQr = (qrToken: string) =>
  fetchApi<Hotel>(`/hotels/qr/${qrToken}`);

// Services APIs
export const getHotelServices = (hotelId: number, lang?: string) =>
  fetchApi<HotelService[]>(
    `/services/hotel/${hotelId}${lang ? `?lang=${lang}` : ""}`,
  );
...
export const sendStaffMessage = (
  ...
  fetchApi<ChatMessage>(
    `/chat/sessions/${sessionId}/messages/staff/${userId}`,
    {
      method: "POST",
      body: JSON.stringify(data),
    },
